...
describe('访谈摘录册（v1.13 门禁 64 / v1.14 门禁 68：扩容至 ≥20 条）', () => {
  it('条目 ≥20，id 唯一，四要素齐全（topic/zh/en/source）', () => {
    ...
    for (const v of INTERVIEWS) {
      ...
      expect(v.source, `${v.id} 缺出处类型`).toMatch(/访谈|著作|讲座/);
    }
  });
...
describe('访谈摘录接线（源码级审计）', () => {
  ...
  it('overlay 提供 showInterviews 面板且引入 INTERVIEWS 数据', () => {
    expect(overlay).toContain('showInterviews()');
    expect(overlay).toMatch(/import \{ INTERVIEWS \} from '\.\.\/data\/interviews\.js'/);
  });
