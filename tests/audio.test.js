...
describe('v1.3 新交互音色（源码审计：合成器 switch 分支存在）', () => {
  ...
  it.each(['bell', 'ratchet', 'creak', 'switch', 'crank', 'projector', 'vinyl', 'doorfar', 'phonering', 'gurgle', 'rain'])('音色 %s 已实现', (name) => {
...
describe('v1.4 阶段 5 新音色（配套阶段 3/4 新交互；源码审计）', () => {
  const src = readFileSync(new URL('../src/audio/engine.js', import.meta.url), 'utf8');
  it.each(['coin', 'springdoor', 'ladderroll', 'flutter', 'stamp', 'iceclink', 'jostle', 'thunder', 'drip', 'coalrattle', 'pluck'])(
    '音色 %s 已实现', (name) => {
      expect(src).toContain(`case '${name}'`);
    }
  );
  ...
  it('新音色已在展厅接线（每种至少一处调用）', () => {
    const halls = ['mulholland', 'archive', 'twinpeaks', 'bluevelvet', 'lobby', 'studio', 'eraserhead']
      ...
      .join('\n');
    for (const name of ['coin', 'springdoor', 'ladderroll', 'flutter', 'stamp', 'iceclink', 'jostle', 'thunder', 'drip', 'coalrattle', 'pluck']) {
      expect(halls).toContain(`'${name}'`);
    }
  });
});

describe('v1.5 新音色（拐角惊吓 / 没有乐队 / 对讲机 / 远景事件；源码审计）', () => {
  const src = readFileSync(new URL('../src/audio/engine.js', import.meta.url), 'utf8');
  it.each(['dread', 'metalscrape', 'aria', 'silencecut', 'walkie', 'sirenfar'])(
    '音色 %s 已实现', (name) => {
      expect(src).toContain(`case '${name}'`);
    }
  );
  ...
  it('新音色已在展厅接线（每种至少一处调用）', () => {
    const halls = ['mulholland', 'twinpeaks']
      .map((h) => readFileSync(new URL(`../src/halls/${h}.js`, import.meta.url), 'utf8'))
      .join('\n');
    for (const name of ['dread', 'metalscrape', 'aria', 'silencecut', 'walkie', 'sirenfar']) {
      expect(halls).toContain(`'${name}'`);
    }
  });
});
