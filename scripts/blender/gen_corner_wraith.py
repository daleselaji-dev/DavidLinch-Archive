...
#
# v1.29 第四轮回炉（用户证词「建模仍不够吓人/不符合原著」——眼组
# 一字不动）：发帘开口再收（OPEN_HALF 0.17→0.14），脸窗只剩发顶与
# 两点红；体色再压；pivot 前倾保持 0.18。
# v1.28 第三轮回炉（用户证词「建模不够吓人/不符合原著」——眼组
# 参数一字不动，形体/发帘/peek 体态全重塑，对齐双峰 BOB / 穆赫兰道
# 拐角「发帘遮脸、肩线先露头、枯瘦前倾」的剪影语言，抽象无肖像）：
#   拍 1  v1.22 定稿四机位重读：peek 体态仍像「站直了滑出来」、
#         发帘开口仍偏宽（脸窗太大不像「只露发顶」）、肩线对称
#   拍 2  peek 剖面/体态：主身再收窄一档 + 冠顶再降 + 佝偻场加深
#         + 肩线 peek 不对称（左肩抬如贴角探看）；发帘开口收窄
#         （OPEN_HALF 0.21→0.17）+ 前垂发绺 4→7 + 侧 peek 绺 ×3
#   拍 3  材质再压黑 + pivot 前倾 0.12→0.18（常态 peek 体态）
#   拍 4  近景：peek 肩 hump 位移场 + 发帘下沿不对称合拢加严
...
# 车削剖面（r, y）——v1.28 拍 2 再收瘦一档（枯瘦 peek 体态）：
BODY_PROFILE = [
    (0.32, 0.0), (0.315, 0.02), (0.28, 0.09), (0.225, 0.22), (0.175, 0.36),
    (0.145, 0.5), (0.132, 0.58), (0.152, 0.68), (0.168, 0.76), (0.138, 0.80),
    (0.078, 0.83), (0.058, 0.85), (0.085, 0.885), (0.095, 0.93),
    (0.055, 0.968), (0.001, 0.978)
]
...
def displace_body(obj, fine=False):
    ...
    for v in obj.data.vertices:
        ...
        stoop = min(1.0, max(0.0, (u - 0.42) / 0.48))
        stoop *= 1.0 - min(1.0, max(0.0, (u - 0.80) / 0.10))
        y = y - hump_b * H * 0.085 - hump_2 * H * 0.035 \
            + head_f * H * 0.05 + stoop * stoop * H * 0.068
        # v1.28 peek 肩：左肩抬、右肩塌——贴角探看的剪影
        peek_sh = math.exp(-((u - 0.76) / 0.05) ** 2)
        z = z - head_f * H * 0.025 + shoulder * (x / (0.2 * H)) * H * 0.032 \
            + peek_sh * (0.022 if x < 0 else -0.014) * H
        v.co = (x, y, z)
...
def build_mid(fine=False):
    ...
    body_mat = make_material(
        'wraithBody', (0.004, 0.003, 0.008, 1), roughness=0.97,
        sheen=0.18, emission=(0.05, 0.004, 0.02, 1), emission_strength=0.035)
...
def build_fine():
    ...
    hair_mat = make_material(
        'wraithHair', (0.003, 0.002, 0.006, 1), roughness=0.88,
        sheen=0.2, emission=(0.01, 0.008, 0.022, 1), emission_strength=0.03)
    ...
    # v1.28 前垂发绺 ×7 + 侧 peek 绺 ×3——帘缝只留发顶/肩线，脸窗
    # 被湿发搭住（BOB 式发帘遮脸，眼组从黑里浮出）
    for fa, fw, flen, fz in ((-0.34, 0.015, 0.52, -0.040), (-0.22, 0.016, 0.38, -0.055),
                             (-0.08, 0.014, 0.48, -0.048), (0.04, 0.013, 0.44, -0.062),
                             (0.16, 0.015, 0.36, -0.058), (0.26, 0.012, 0.50, -0.050),
                             (0.34, 0.014, 0.32, -0.068)):
        rr = H * 0.102
        loc = (math.sin(fa) * rr, math.cos(fa) * rr, H * (0.84 + fz))
        rot = (math.cos(fa) * 0.08 + (rnd() - 0.5) * 0.04,
               -math.sin(fa) * 0.08 + (rnd() - 0.5) * 0.04, 0)
        cones.append((fw * H, flen * H, loc, rot))
    for fa, fw, flen, fz in ((-0.55, 0.011, 0.28, 0.02), (0.58, 0.012, 0.24, 0.015),
                             (0.72, 0.010, 0.20, -0.01)):
        rr = H * 0.118
        loc = (math.sin(fa) * rr, math.cos(fa) * rr, H * (0.84 + fz))
        rot = (math.cos(fa) * 0.11, -math.sin(fa) * 0.09, (rnd() - 0.5) * 0.08)
        cones.append((fw * H, flen * H, loc, rot))
...
def main():
    ...
    for obj in list(bpy.context.collection.objects):
        if obj.type == 'MESH':
            obj.parent = pivot
    pivot.rotation_euler = (0.18, 0.08, 0)
